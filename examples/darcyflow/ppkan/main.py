...
def train(cfg: DictConfig, with_val=False):
    ...
    optimizer = paddle.optimizer.Adam(parameters=model.parameters(), learning_rate=cfg.TRAIN.lr, weight_decay=1e-4)
    if cfg.enable_ddp:
    ...
    error_msg = f"training epochs {cfg.TRAIN.epochs} should be greater than resume epoch, " f"which is {resume_ep} now."
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            ...
            paddle.save(model.state_dict(), f"{cfg.output_dir}/{cfg.model}_latest.pdparams")
            paddle.save(optimizer.state_dict(), f"{cfg.output_dir}/{cfg.model}_latest.pdopt")
        scheduler.step()
    end_time = time.time()
    ...
    with paddle.no_grad():
        ...
        # Add title
        axs[0].set_title("Bspline-KAN-Predicted")
    ...
    # Save loss and duration
    np.savetxt(os.path.join(cfg.output_dir, "train_losses.txt"), train_losses, fmt="%.6f")
    np.savetxt(os.path.join(cfg.output_dir, "test_losses.txt"), test_losses, fmt="%.6f")
    logging.info(f"Training completed in {training_time:.2f} seconds.")
    logging.info(f"Final Train Loss: {train_losses[-1]:.6f}, Final Test Loss: {test_losses[-1]:.6f}")
...
def main(cfg: DictConfig):
    ...
    if cfg.mode == "train":
        ...
        raise ValueError(f"cfg.mode should in ['train', 'valid', 'test'], but got '{cfg.mode}'")
